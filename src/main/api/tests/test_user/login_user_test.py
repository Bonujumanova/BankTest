...
class TestLoginUser:
    ...
    def test_login_user_valid(self):
        create_user_request = CreateUserRequest(username="Max105", password="Pas!sw0rd", role="ROLE_USER")
        CreateUserRequester(
        ...
        login_user_request = LoginUserRequest(username="Max105", password="Pas!sw0rd")
        response = LoginUserRequester(
            request_spec=RequestSpecs.authorization_headers(username="admin", password="123456"),
            response_spec=ResponseSpecs.request_ok()
        ).post(login_user_request)
